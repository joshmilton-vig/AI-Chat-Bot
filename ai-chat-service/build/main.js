...
var VividAssistant = (() => {
      ...
      if ((t && typeof t == "object") || typeof t == "function")
        for (let a of W(t))
          !Y.call(e, a) &&
            ...
            T(e, a, {
              get: () => t[a],
              enumerable: !(r = R(t, a)) || r.enumerable,
            });
      return e;
    };
  var Q = (e) => q(T({}, "__esModule", { value: !0 }), e);
  var ie = {};
  J(ie, { mountVividChat: () => se });
  async function N(e, t, i, r) {
    let a = await fetch(`${e}/chat`, {
      method: "POST",
    ...
    if (!a.ok) {
      let g = await a.text().catch(() => "");
      throw new Error(`Chat error ${a.status}: ${g || a.statusText}`);
    }
    let x = await a.json();
    return String(x.message ?? "Sorry \u2014 I couldn\u2019t get a response.");
  }
  var B = "vivid-chat-style",
    n = "vivid-chat-widget",
    H = "vivid-chat-host",
    z = null;
  function K() {
    ...
    return (
      ...
      (z = e.shadowRoot ?? e.attachShadow({ mode: "open" })),
      { host: e, shadow: z }
    );
  }
  function U(e) {
    let { shadow: t } = K(),
      i = t.getElementById(B);
    i && i.remove();
    let r = `
  ...
  `,
    ...
    (a.id = B), (a.textContent = r), t.appendChild(a);
  }
  function X(e) {
  ...
  function V(e = {}) {
    let t = {
      ...
      i = t.apiBase ?? "/api/ai",
      r = t.business ?? ne(),
      a = t.siteName ?? (document.title || "Vivid Store"),
      x = !!t.debug,
      g = X(t.storageKey),
      I = "vivid_chat_active_session";
    sessionStorage.getItem(I) || localStorage.removeItem(g),
    ...
    let j = {
        ...
        "--va-header-bg": t.theme?.headerBg || "#000",
        "--va-header-text": t.theme?.headerText || "#fff",
    ...
    (m.className = "bubble"),
      (m.title = "Chat"),
      (m.innerHTML = t.iconSVG || te());
    let p = document.createElement("div");
    p.className = "panel";
    let C = document.createElement("header"),
      E = document.createElement("div");
    (E.className = "title"), (E.textContent = t.title || "Prisma Assistant");
    ...
    f.placeholder = "Ask about products, lead times, shipping, etc.";
    ...
    (S.className = "hint"),
      ...
      P.appendChild(y);
    function $(s) {
      if (((v = s), p.classList.toggle("open", v), v && t.welcomeMessage)) {
        let o = "vivid_chat_welcome_shown";
        (!t.welcomeOnce || !D(o)) &&
          (b("assistant", t.welcomeMessage), t.welcomeOnce && A(o));
      }
    }
    m.addEventListener("click", () => $(!v)),
      w.addEventListener("click", () => $(!1));
    let d = (t.persist && Z(g)) || [{ role: "system", content: ae(a, r) }];
    if (d.length && d[0].role === "system")
      for (let s of d.slice(1)) b(s.role, s.content);
    function k() {
      t.persist && ee(g, d);
    }
    let l = null;
    ...
    async function M() {
      ...
      if (s) {
        ...
        try {
          ...
          L(),
            (u.disabled = !1),
            d.push({ role: "assistant", content: o }),
            b("assistant", o),
            k(),
            G("vivid_chat_message", { role: "assistant" });
        } catch (o) {
          L(), (u.disabled = !1);
    ...
    function G(s, o) {
      (window.dataLayer = window.dataLayer || []),
        window.dataLayer.push({ event: s, ...o, business: r, siteName: a });
    }
    if (
      (x &&
